...
dir <- ("K:/Google Drive/DOUTORADO/Tese Cinthia/Modelagem/")

# Creating necessary folders ----

dir.create(paste0(dir, "RF/"))
dir.create(paste0(dir, "RF/Tables"))
dir.create(paste0(dir, "RF/Tables/validation"))
dir.create(paste0(dir, "RF/Tables/calibration"))
dir.create(paste0(dir, "RF/figures"))
dir.create(paste0(dir, "RF/models"))
dir.create(paste0(dir, "PLS/"))
dir.create(paste0(dir, "PLS/Tables"))
dir.create(paste0(dir, "PLS/Tables/validation"))
dir.create(paste0(dir, "PLS/Tables/calibration"))
dir.create(paste0(dir, "PLS/figures"))
dir.create(paste0(dir, "PLS/models"))

# Importing and formatting dataset ----

dataset = read.csv(paste0(dir, "dataset.csv"))
...
c = 3 #first column with attribute value
v = 40 #last column with attribute value

summary(dataset[, c:v])

filtro = dataset %>% drop_na(X1000)
database = filtro

database[, c:v] = sapply(database[, c:v], as.numeric)
summary(database[, c:v])

write.csv(summary(filtro[,c:v]), paste0(dir, "attributes_summary.csv"))
print("Attributes written")

#_________________________________----
# Sand, silt and clay) ----
{
  # Defining variables and attribute range
  
  str(database[,1:45])
  colnames(database[,1:45])
  
  db = cbind(database[,1:5], database[,(v+1):length(database)])
  
  colnames(database[,1:15])
  
  print("WARNING! The following variables must be inserted manually")
  print("The insertion must be based on the first and last columns with elemental concentrations")
  print("The following numbers correspond to the index columns from the example dataset")
  
  i = 3 #first column with attribute value
  j = 5 #last column with attribute value
  ...
  db.ok.res = cbind(db.ok[,1], db.ok[, i:j], db.res[,2:1502])
  str(db.ok.res[,1:30])
  colnames(db.ok.res[,1:30])
  colnames(db.ok.res)[1] = "id"
  ...
  db.train.msc = prospectr::msc(db.train[, (j+1):length(names(db.train))])
  class(db.train.msc)
  ...
  db.train.msc = cbind(db.train[,1:(j)], db.train.msc[,2:length(db.train.msc)])
  ...
  db.val.msc = cbind(db.val[,1:(j)], db.val.msc[,2:length(db.val.msc)])
  head(db.val.msc[, c(1:6, length(db.val.msc))])
  ...
  db.train.sg = savitzkyGolay(db.train[, (j+1):length(names(db.train))], m = 1, p = 1, w=9)
  class(db.train.sg)
  ...
  db.train.sg = cbind(db.train[,1:(j)], db.train.sg[,2:length(db.train.sg)])
  head(db.train.sg[, c(1:6, length(db.train.sg))])
  
  ### Validation data
  colnames(db.val[,1:30])
  db.val.sg = savitzkyGolay(db.val[, (j+1):length(names(db.train))], m = 1, p = 1, w=9)
  class(db.val.sg)
  ...
  db.val.sg = cbind(db.val[,1:(j)], db.val.sg[,2:length(db.val.sg)])
  saveRDS(db.val.sg, paste0(dir, " db_val.SG.rda"))
  
  ## Standard Normal Variate
  ### Training data
  colnames(db.train[, 1:30])
  db.train.snv = standardNormalVariate(db.train[, (j+1):length(names(db.train))])
  class(db.train.snv)
  ...
  db.train.snv = cbind(db.train[,1:(j)], db.train.snv[,2:length(db.train.snv)])
  head(db.train.snv[, c(1:6, length(db.train.snv))])
  
  ### Validation data
  colnames(db.val[,1:30])
  db.val.snv = standardNormalVariate(db.val[, (j+1):length(names(db.val))])
  class(db.val.snv)
  ...
  db.val.snv = cbind(db.val[,1:(j)], db.val.snv[,2:length(db.val.snv)])
  head(db.val.snv[, c(1:6, length(db.val.snv))])
  ...
  ### Training data
  colnames(db.train[, 1:30])
  wav.band.det = as.numeric(colnames(db.train[, (j+1):length(names(db.train))]))
  db.train.det = detrend(db.train[, (j+1):length(names(db.train))], wav.band.det, p = 2)
  class(db.train.det)
  ...
  db.train.det = cbind(db.train[,1:(j)], db.train.det[,2:length(db.train.det)])
  head(db.train.det[, c(1:6, length(db.train.det))])
  
  ### Validation data
  colnames(db.val[,1:30])
  db.val.det = detrend(db.val[, (j+1):length(names(db.val))], wav.band.det, p = 2)
  class(db.val.det)
  ...
  db.val.det = cbind(db.val[,1:(j)], db.val.det[,2:length(db.val.det)])
  head(db.val.det[, c(1:6, length(db.val.det))])
  ...
  ### Training data
  colnames(db.train[, 1:30])
  wav.band.cr = as.numeric(colnames(db.train[, (j+1):length(names(db.train))]))
  db.train.cr = continuumRemoval(db.train[, (j+1):length(names(db.train))], 
                                 wav.band.cr, 
  ...
  db.train.cr = cbind(db.train[,1:(j)], db.train.cr[,2:length(db.train.cr)])
  head(db.train.cr[, c(1:6, length(db.train.cr))])
  
  ### Validation data
  colnames(db.val[,1:30])
  db.val.cr = continuumRemoval(db.val[, (j+1):length(names(db.val))], 
                               wav.band.cr, 
                               type = "R", interpol = 'linear', 
                               method = 'division')
  class(db.val.cr)
  ...
  db.val.cr = cbind(db.val[,1:(j)], db.val.cr[,2:length(db.val.cr)])
  head(db.val.cr[, c(1:6, length(db.val.cr))])
  ...
  for (w in i:j){
    ...
    system.time(Orig.Rf.Model <- train(x=db.train[ ,(j+1):length(names(db.train))], 
                                       y=db.train[,ktrain],
                                       method = "rf", 
                                       trControl = ctrl,
                                       importance = T))  
    ...
    saveRDS(Orig.Rf.Model, file = paste0(dir, 'RF/models/', paste(ElementCode, "Orig.Rf.Model.rda", sep=".")))
    ...
    colnames(db.val[, 1:30])
    db.val[orig.dbval] = stats::predict(Orig.Rf.Model, db.val[, (j+1):length(names(db.val))])
    ...
    jpeg(paste0(dir, 'RF/figures/', paste(ElementCode, '_Orig_Obs_Pred.jpeg', sep="")), 
         width = 10, height = 10, units = 'in', res = 300)
    par(mar = c(6, 6, 6, 6))
    summary(db.val[, c(ktrain, length(db.val))])
    plot(db.val[,orig.dbval], db.val[,ktrain],
         xlab = expression(paste('Predicted ', '(', 'g', ' kg'^'-1',')')),
         ylab = expression(paste('Observed ', '(', 'g', ' kg'^'-1',')')),
         xlim = c(0, MVXX),
         ylim = c(0, MVXX),
         type = 'p',
         pch = 16,
    ...
    system.time(CR.Rf.Model <- train(x=db.train.cr[ ,(j+1):length(names(db.train.cr))], 
    ...
    saveRDS(CR.Rf.Model, file = paste0(dir, 'RF/models/', paste(ElementCode, "CR.Rf.Model.rda", sep=".")))
    ...
    colnames(db.val.cr[, 1:30])
    db.val.cr[CR.dbval] = stats::predict(CR.Rf.Model, db.val.cr[, (j+1):length(names(db.val.cr))])
    # Predicting using externaldata
    ...
    jpeg(paste0(dir, 'RF/figures/', paste(ElementCode, '_CR_Obs_Pred.jpeg', sep="")), 
         width = 10, height = 10, units = 'in', res = 300)
    par(mar = c(6, 6, 6, 6))
    ...
    system.time(SG.Rf.Model <- train(x=db.train.sg[ ,(j+1):length(names(db.train.sg))], 
    ...
    #dir.create(paste0(dir, "models"))
    saveRDS(SG.Rf.Model, file = paste0(dir, 'RF/models/', paste(ElementCode, "SG.Rf.Model.rda", sep=".")))
    ...
    colnames(db.val.sg[, 1:30])
    db.val.sg[SG.dbval] = stats::predict(SG.Rf.Model, db.val.sg[, (j+1):length(names(db.val.sg))])
    # Predicting using externaldata
    ...
    jpeg(paste0(dir, 'RF/figures/', paste(ElementCode, '_SG_Obs_Pred.jpeg', sep="")), 
         width = 10, height = 10, units = 'in', res = 300)
    par(mar = c(6, 6, 6, 6))
    ...
    system.time(SNV.Rf.Model <- train(x=db.train.snv[ ,(j+1):length(names(db.train.snv))], 
    ...
    #dir.create(paste0(dir, "models"))
    saveRDS(SNV.Rf.Model, file = paste0(dir, 'RF/models/', paste(ElementCode, "SNV.Rf.Model.rda", sep=".")))
    ...
    colnames(db.val.snv[, 1:30])
    db.val.snv[SNV.dbval] = stats::predict(SNV.Rf.Model, db.val.snv[, (j+1):length(names(db.val.snv))])
    # Predicting using externaldata
    ...
    jpeg(paste0(dir, 'RF/figures/', paste(ElementCode, '_SNV_Obs_Pred.jpeg', sep="")), 
         width = 10, height = 10, units = 'in', res = 300)
    par(mar = c(6, 6, 6, 6))
    ...
    system.time(DET.Rf.Model <- train(x=db.train.det[ ,(j+1):length(names(db.train.det))], 
    ...
    #dir.create(paste0(dir, "models"))
    saveRDS(DET.Rf.Model, file = paste0(dir, 'RF/models/', paste(ElementCode, "DET.Rf.Model.rda", sep=".")))
    ...
    colnames(db.val.det[, 1:30])
    db.val.det[DET.dbval] = stats::predict(DET.Rf.Model, db.val.det[, (j+1):length(names(db.val.det))])
    # Predicting using externaldata
    ...
    jpeg(paste0(dir, 'RF/figures/', paste(ElementCode, '_DET_Obs_Pred.jpeg', sep="")), 
         width = 10, height = 10, units = 'in', res = 300)
    par(mar = c(6, 6, 6, 6))
    ...
    system.time(MSC.Rf.Model <- train(x=db.train.msc[ ,(j+1):length(names(db.train.msc))], 
    ...
    #dir.create(paste0(dir, "models"))
    saveRDS(MSC.Rf.Model, file = paste0(dir, 'RF/models/', paste(ElementCode, "MSC.Rf.Model.rda", sep=".")))
    ...
    colnames(db.val.msc[, 1:30])
    db.val.msc[MSC.dbval] = stats::predict(MSC.Rf.Model, db.val.msc[, (j+1):length(names(db.val.msc))])
    # Predicting using externaldata
    ...
    jpeg(paste0(dir, 'RF/figures/', paste(ElementCode, '_MSC_Obs_Pred.jpeg', sep="")), 
         width = 10, height = 10, units = 'in', res = 300)
    par(mar = c(6, 6, 6, 6))
    ...
    write.csv(cal.res, paste0(dir, 'RF/Tables/calibration/', paste(ElementCode, "calibration.csv", sep="_")), 
              row.names = FALSE)
    ...
    write.csv(res, paste0(dir, 'RF/Tables/validation/', paste(ElementCode, "validation.csv", sep="_")), 
              row.names = FALSE)
    
    
  }
  # Partial Least Squares modeling and prediction ----
  #
  ctrl = caret::trainControl(method = 'cv', number = 10, search = 'grid')
  cores = parallel::detectCores()-2
  #
  ...
  for (w in i:j){
    ...
    system.time(Orig.PLS.Model <- train(x=db.train[ ,(j+1):length(names(db.train))], 
                                        y=db.train[,ktrain],
                                        method = "pls", 
                                        trControl = ctrl,
                                        importance = T))  
    ...
    saveRDS(Orig.PLS.Model, file = paste0(dir, 'PLS/models/', paste(ElementCode, "Orig.PLS.Model.rda", sep=".")))
    
    print(Orig.PLS.Model)
    assign(paste(ElementCode, "Orig.PLS.Model", sep="."), Orig.PLS.Model)
    
    Orig.PLS.Model.Variables = varImp(Orig.PLS.Model, scale = T)
    
    
    orig.dbval = length(db.val)+1
    colnames(db.val[, 1:30])
    db.val[orig.dbval] = stats::predict(Orig.PLS.Model, db.val[, (j+1):length(names(db.val))])
    # Predicting using externaldata
    ...
    jpeg(paste0(dir, 'PLS/figures/', paste(ElementCode, '_Orig_Obs_Pred.jpeg', sep="")), 
         width = 10, height = 10, units = 'in', res = 300)
    par(mar = c(6, 6, 6, 6))
    summary(db.val[, c(ktrain, length(db.val))])
    plot(db.val[,orig.dbval], db.val[,ktrain],
    ...
    system.time(CR.PLS.Model <- train(x=db.train.cr[ ,(j+1):length(names(db.train.cr))], 
                                      y=db.train.cr[,ktrain],
                                      method = "pls", 
                                      trControl = ctrl,
                                      importance = T))  
    ...
    #dir.create(paste0(dir, "models"))
    saveRDS(CR.PLS.Model, file = paste0(dir, 'PLS/models/', paste(ElementCode, "CR.PLS.Model.rda", sep=".")))
    
    print(CR.PLS.Model)
    assign(paste(ElementCode, "CR.PLS.Model", sep="."), CR.PLS.Model)
    
    CR.PLS.Model.Variables = varImp(CR.PLS.Model, scale = T)
    
    CR.dbval = length(db.val.cr)+1
    colnames(db.val.cr[, 1:30])
    db.val.cr[CR.dbval] = stats::predict(CR.PLS.Model, db.val.cr[, (j+1):length(names(db.val.cr))])
    # Predicting using externaldata
    ...
    jpeg(paste0(dir, 'PLS/figures/', paste(ElementCode, '_CR_Obs_Pred.jpeg', sep="")), 
         width = 10, height = 10, units = 'in', res = 300)
    par(mar = c(6, 6, 6, 6))
    summary(db.val.cr[, c(ktrain, length(db.val.cr))])
    plot(db.val.cr[,CR.dbval], db.val.cr[,ktrain],
    ...
    system.time(SG.PLS.Model <- train(x=db.train.sg[ ,(j+1):length(names(db.train.sg))], 
                                      y=db.train.sg[,ktrain],
                                      method = "pls", 
                                      trControl = ctrl,
                                      importance = T))  
    ...
    #dir.create(paste0(dir, "models"))
    saveRDS(SG.PLS.Model, file = paste0(dir, 'PLS/models/', paste(ElementCode, "SG.PLS.Model.rda", sep=".")))
    
    print(SG.PLS.Model)
    assign(paste(ElementCode, "SG.PLS.Model", sep="."), SG.PLS.Model)
    
    SG.PLS.Model.Variables = varImp(SG.PLS.Model, scale = T)
    
    SG.dbval = length(db.val.sg)+1
    colnames(db.val.sg[, 1:30])
    db.val.sg[SG.dbval] = stats::predict(SG.PLS.Model, db.val.sg[, (j+1):length(names(db.val.sg))])
    # Predicting using externaldata
    ...
    jpeg(paste0(dir, 'PLS/figures/', paste(ElementCode, '_SG_Obs_Pred.jpeg', sep="")), 
         width = 10, height = 10, units = 'in', res = 300)
    par(mar = c(6, 6, 6, 6))
    summary(db.val.sg[, c(ktrain, length(db.val.sg))])
    plot(db.val.sg[,SG.dbval], db.val.sg[,ktrain],
    ...
    system.time(SNV.PLS.Model <- train(x=db.train.snv[ ,(j+1):length(names(db.train.snv))], 
                                       y=db.train.snv[,ktrain],
                                       method = "pls", 
                                       trControl = ctrl,
                                       importance = T))  
    ...
    #dir.create(paste0(dir, "models"))
    saveRDS(SNV.PLS.Model, file = paste0(dir, 'PLS/models/', paste(ElementCode, "SNV.PLS.Model.rda", sep=".")))
    
    print(SNV.PLS.Model)
    assign(paste(ElementCode, "SNV.PLS.Model", sep="."), SNV.PLS.Model)
    
    SNV.PLS.Model.Variables = varImp(SNV.PLS.Model, scale = T)
    
    SNV.dbval = length(db.val.snv)+1
    colnames(db.val.snv[, 1:30])
    db.val.snv[SNV.dbval] = stats::predict(SNV.PLS.Model, db.val.snv[, (j+1):length(names(db.val.snv))])
    # Predicting using externaldata
    ...
    jpeg(paste0(dir, 'PLS/figures/', paste(ElementCode, '_SNV_Obs_Pred.jpeg', sep="")), 
         width = 10, height = 10, units = 'in', res = 300)
    par(mar = c(6, 6, 6, 6))
    summary(db.val.snv[, c(ktrain, length(db.val.snv))])
    plot(db.val.snv[,SNV.dbval], db.val.snv[,ktrain],
    ...
    system.time(DET.PLS.Model <- train(x=db.train.det[ ,(j+1):length(names(db.train.det))], 
                                       y=db.train.det[,ktrain],
                                       method = "pls", 
                                       trControl = ctrl,
                                       importance = T))  
    ...
    #dir.create(paste0(dir, "models"))
    saveRDS(DET.PLS.Model, file = paste0(dir, 'PLS/models/', paste(ElementCode, "DET.PLS.Model.rda", sep=".")))
    
    print(DET.PLS.Model)
    assign(paste(ElementCode, "DET.PLS.Model", sep="."), DET.PLS.Model)
    
    DET.PLS.Model.Variables = varImp(DET.PLS.Model, scale = T)
    
    DET.dbval = length(db.val.det)+1
    colnames(db.val.det[, 1:30])
    db.val.det[DET.dbval] = stats::predict(DET.PLS.Model, db.val.det[, (j+1):length(names(db.val.det))])
    # Predicting using externaldata
    ...
    jpeg(paste0(dir, 'PLS/figures/', paste(ElementCode, '_DET_Obs_Pred.jpeg', sep="")), 
         width = 10, height = 10, units = 'in', res = 300)
    par(mar = c(6, 6, 6, 6))
    summary(db.val.det[, c(ktrain, length(db.val.det))])
    plot(db.val.det[,DET.dbval], db.val.det[,ktrain],
    ...
    system.time(MSC.PLS.Model <- train(x=db.train.msc[ ,(j+1):length(names(db.train.msc))], 
                                       y=db.train.msc[,ktrain],
                                       method = "pls", 
                                       trControl = ctrl,
                                       importance = T))  
    ...
    #dir.create(paste0(dir, "models"))
    saveRDS(MSC.PLS.Model, file = paste0(dir, 'PLS/models/', paste(ElementCode, "MSC.PLS.Model.rda", sep=".")))
    
    print(MSC.PLS.Model)
    assign(paste(ElementCode, "MSC.PLS.Model", sep="."), MSC.PLS.Model)
    
    MSC.PLS.Model.Variables = varImp(MSC.PLS.Model, scale = T)
    
    MSC.dbval = length(db.val.msc)+1
    colnames(db.val.msc[, 1:30])
    db.val.msc[MSC.dbval] = stats::predict(MSC.PLS.Model, db.val.msc[, (j+1):length(names(db.val.msc))])
    # Predicting using externaldata
    ...
    jpeg(paste0(dir, 'PLS/figures/', paste(ElementCode, '_MSC_Obs_Pred.jpeg', sep="")), 
         width = 10, height = 10, units = 'in', res = 300)
    par(mar = c(6, 6, 6, 6))
    summary(db.val.msc[, c(ktrain, length(db.val.msc))])
    plot(db.val.msc[,MSC.dbval], db.val.msc[,ktrain],
    ...
    orig = Orig.PLS.Model$results[Orig.PLS.Model$results$ncomp %in% Orig.PLS.Model$bestTune, ]
    orig$Variable = paste(ElementCode); orig$Preproc = 'Raw Spectra'
    
    snv = SNV.PLS.Model$results[SNV.PLS.Model$results$ncomp %in% SNV.PLS.Model$bestTune, ]
    snv$Variable = paste(ElementCode); snv$Preproc = 'Standard Normal Variate'
    
    cr = CR.PLS.Model$results[CR.PLS.Model$results$ncomp %in% CR.PLS.Model$bestTune, ]
    cr$Variable = paste(ElementCode); cr$Preproc = 'Continuum Removal'
    
    det = DET.PLS.Model$results[DET.PLS.Model$results$ncomp %in% DET.PLS.Model$bestTune, ]
    det$Variable = paste(ElementCode); det$Preproc = 'Detrend normalization'
    
    sg = SG.PLS.Model$results[SG.PLS.Model$results$ncomp %in% SG.PLS.Model$bestTune, ]
    sg$Variable = paste(ElementCode); sg$Preproc = 'Savitzky-Golay'
    
    msc = MSC.PLS.Model$results[MSC.PLS.Model$results$ncomp %in% MSC.PLS.Model$bestTune, ]
    msc$Variable = paste(ElementCode); msc$Preproc = 'Multiplicative Scatter Correction'
    ...
    write.csv(cal.res, paste0(dir, 'PLS/Tables/calibration/', paste(ElementCode, "calibration.csv", sep="_")), 
              row.names = FALSE)
    ...
    write.csv(res, paste0(dir, 'PLS/Tables/validation/', paste(ElementCode, "validation.csv", sep="_")), 
              row.names = FALSE)
    
    
  }
  # info table ----
  info = as.data.frame(table(db.ok$UF))
  info = as.data.frame(t(info))
  info$samples = c("samples", paste(nrow(db.ok)))
  info$calibration = c("calibration", paste(nrow(db.train)))
  info$validation = c("validation", paste(nrow(db.val)))
  colnames(info) = info[c(1),]
  info = info[2,]
  
  write.csv(info, paste0(dir, "RF/", paste(ElementCode, "info.csv", sep = "_")), row.names = FALSE)
...
# pH ----
{
  ...
  db = cbind(database[,1:2], database[,6], database[,(v+1):length(database)])
  
  colnames(database[,1:15])
  
  colnames(db)[3] = colnames(database)[6]
  ...
  i = 3 #first column with attribute value
  j = 3 #last column with attribute value
  ...
  write.csv(db.ok.res, paste0(dir, 'db_resampled.csv'), row.names = F)
  print("db_resampled.csv Written.")
  
  # Splitting dataset into test and validation using conditioned Latin Hypercube Sampling (cLHS) ----
  colnames(db.ok.res)
  
  # the splitting will be made at a 80 to 20 calibration/validation ratio
  ree.clhs = clhs(db.ok.res[, (j):length(db.ok.res)],
                  size = as.integer(0.8*nrow(db.ok.res)),
                  iter = 1000, progress = FALSE,
                  simple = FALSE)
  print("cLHS splitting completed!")
  ...
  for (w in i:j){
    ...
    summary(db.val[, c(ktrain, length(db.val))])
    plot(db.val[,orig.dbval], db.val[,ktrain],
         xlab = expression(paste('Predicted ', '(', 'pH',')')),
         ylab = expression(paste('Observed ', '(', 'pH',')')),
         xlim = c(0, MVXX),
         ylim = c(0, MVXX),
         type = 'p',
         pch = 16,
...
# CTC ----
{
  ...
  db = cbind(database[,1:2], database[,7], database[,(v+1):length(database)])
  
  colnames(database[,1:15])
  
  colnames(db)[3] = colnames(database)[7]
  ...
  for (w in i:j){
    ...
    summary(db.val[, c(ktrain, length(db.val))])
    plot(db.val[,orig.dbval], db.val[,ktrain],
         xlab = expression(paste('Predicted ', '(', 'cmolc', ' dm'^'3',')')),
         ylab = expression(paste('Observed ', '(', 'cmolc', ' dm'^'3',')')),
         xlim = c(0, MVXX),
         ylim = c(0, MVXX),
         type = 'p',
         pch = 16,
  ...
  for (w in i:j){
    ...
    orig = Orig.Rf.Model$results[Orig.Rf.Model$results$ncomp %in% Orig.Rf.Model$bestTune, ]
    orig$Variable = paste(ElementCode); orig$Preproc = 'Raw Spectra'
    
    snv = SNV.Rf.Model$results[SNV.Rf.Model$results$ncomp %in% SNV.Rf.Model$bestTune, ]
    snv$Variable = paste(ElementCode); snv$Preproc = 'Standard Normal Variate'
    
    cr = CR.Rf.Model$results[CR.Rf.Model$results$ncomp %in% CR.Rf.Model$bestTune, ]
    cr$Variable = paste(ElementCode); cr$Preproc = 'Continuum Removal'
    
    det = DET.Rf.Model$results[DET.Rf.Model$results$ncomp %in% DET.Rf.Model$bestTune, ]
    det$Variable = paste(ElementCode); det$Preproc = 'Detrend normalization'
    
    sg = SG.Rf.Model$results[SG.Rf.Model$results$ncomp %in% SG.Rf.Model$bestTune, ]
    sg$Variable = paste(ElementCode); sg$Preproc = 'Savitzky-Golay'
    
    msc = MSC.Rf.Model$results[MSC.Rf.Model$results$ncomp %in% MSC.Rf.Model$bestTune, ]
    msc$Variable = paste(ElementCode); msc$Preproc = 'Multiplicative Scatter Correction'
...
# COT ----
{
  ...
  db = cbind(database[,1:2], database[,8], database[,(v+1):length(database)])
  
  colnames(database[,1:15])
  
  colnames(db)[3] = colnames(database)[8]
...
# Elements in mg kg-1 ----
{
  # Loop start for all elements in mg kg-1
  ...
  for (g in ii:v){
    ...
    db = cbind(database[,1:2], database[,g], database[,(v+1):length(database)])
    
    colnames(database[,1:15])
    
    colnames(db)[3] = colnames(database)[g]
...
library(dplyr)
library(readr)
calibration <- list.files(path=paste0(dir, 'RF/Tables/calibration'), full.names = TRUE) %>% 
  lapply(read.csv) %>% 
  bind_rows
write.csv(calibration,paste0(dir, 'RF/calibration.csv'), row.names = FALSE)

validation <- list.files(path=paste0(dir, 'RF/Tables/validation'), full.names = TRUE) %>% 
  lapply(read.csv) %>% 
  bind_rows
write.csv(validation, paste0(dir, 'RF/validation.csv'), row.names = FALSE)

calibration <- list.files(path=paste0(dir, 'PLS/Tables/calibration'), full.names = TRUE) %>% 
  lapply(read.csv) %>% 
  bind_rows
write.csv(calibration,paste0(dir, 'PLS/calibration.csv'), row.names = FALSE)

validation <- list.files(path=paste0(dir, 'PLS/Tables/validation'), full.names = TRUE) %>% 
  lapply(read.csv) %>% 
  bind_rows
write.csv(validation, paste0(dir, 'PLS/validation.csv'), row.names = FALSE)
### ------------------ ###
### ------------------ ###
#_________________________________----